...
def merge_and_calculate_metrics(results_csv_path, meta_csv_path):
    ...
    # Print the number of male and female speakers
    gender_counts = bvcc_df['gender'].value_counts()
    ...
    # Plot MOS scores by gender
    plot_mos_scores_by_gender(merged_df)
...
def plot_mos_scores_by_gender(merged_df):
    """
    Plot ground truth MOS and predicted MOS scores by gender.
    """
    genders = merged_df['gender'].unique()
    ...
    for gender in genders:
        ...
        # Create a histogram for MOS score versus count of samples
        plt.figure()
        plt.hist(true_mean_scores, bins=20, alpha=0.7, label='Ground Truth MOS')
        plt.hist(predict_mean_scores, bins=20, alpha=0.7, label='Predicted MOS')
        plt.title(f'MOS Scores vs. Count by Gender: {gender}')
        ...
        # Save the figure
        plt.savefig(f'mos_scores_histogram_gender_{gender}.png')
        plt.show()
