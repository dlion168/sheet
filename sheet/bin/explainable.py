...
# ----------------------------------------
# IG, Saliency, Lime, SHAP, and now InputXGradient
# ----------------------------------------
...
def inputxgradient_explanation(model, inputs):
    """
    Uses InputXGradient from captum, which multiplies the input by the gradient of the output
    w.r.t. the input to measure feature importance.
    """
    inputs["waveform"].requires_grad_(True)
...
def main():
    ...
    # === 新增: 可指定 baseline 音檔給 IG 使用 ===
    parser.add_argument(
        "--baseline-wav", 
        type=str, 
        default=None, 
        help="Path to a reference audio file as baseline for Integrated Gradients."
    )
    ...
    # === 若需要 baseline_wav，先載入該音檔（不做長度對齊，留到使用時再做） ===
    ...
    if args.baseline_wav is not None:
        ...
        # 若取樣率與資料集不同，resample
        if sr_baseline != config["sampling_rate"]:
            logging.info(f"Resampling baseline from {sr_baseline} to {config['sampling_rate']}...")
            resampler = torchaudio.transforms.Resample(sr_baseline, config["sampling_rate"])
            baseline_wav = resampler(baseline_wav)
        baseline_wav = baseline_wav.squeeze(-1)  # [T]
        baseline_wav_tensor = baseline_wav.float().to(device)  # 先放到 device
    ...
    gradcam = GradCAM(model, target_layer_idx=0) if args.explain_method == "gradcam" else None
    ...
    for i, batch in enumerate(tqdm(dataset)):
        ...
        # Ground truth 分數 (依您 CSV/資料結構；這裡假設名為 "score" 或 "avg_score")
        ...
        time_axis = np.arange(wave_len) / float(config["sampling_rate"])
        ...
        if args.explain_method == "gradcam":
            ...
            # 若有指定 baseline_wav，則用該檔；否則用 zeros_like
            if baseline_wav_tensor is not None:
                # 把 baseline_wav_tensor 做一份拷貝，再對齊長度
                baseline_wav_matched = match_length(baseline_wav_tensor, wave_len)
                baseline_wav_matched = baseline_wav_matched.unsqueeze(0)  # shape [1, time]
                baseline_waveform = baseline_wav_matched
            else:
